...
Mat filterContrastBrightness(const Mat& sourceimg, double contrast, double brightness)
{
  ...
  normalize(filtered, filtered, 0, 65535, NORM_MINMAX);
...
Mat addnoise(const Mat& sourceimgIn)
{
  ...
  normalize(noised, noised, 0, 255, NORM_MINMAX);
...
void showhistogram(const Mat& sourceimgIn, int channels, int minimum, int maximum, std::string winname)
{
  ...
  {
    line(histImage, Point2i(bin_w * (i - 1), hist_h - cvRound(b_hist.at<float>(i - 1))), Point2i(bin_w * (i), hist_h - cvRound(b_hist.at<float>(i))), Scalar(255, 0, 0), thickness, 8, 0);
    line(histImage, Point2i(bin_w * (i - 1), hist_h - cvRound(g_hist.at<float>(i - 1))), Point2i(bin_w * (i), hist_h - cvRound(g_hist.at<float>(i))), Scalar(0, 255, 0), thickness, 8, 0);
    line(histImage, Point2i(bin_w * (i - 1), hist_h - cvRound(r_hist.at<float>(i - 1))), Point2i(bin_w * (i), hist_h - cvRound(r_hist.at<float>(i))), Scalar(0, 0, 255), thickness, 8, 0);
  }
