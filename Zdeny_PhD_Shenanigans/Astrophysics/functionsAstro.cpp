...
DiffrotResults2 calculateDiffrotProfile( const IPCsettings &set, FitsTime &time, int itersPic, int itersX, int itersY, int itersMedian, int strajdPic, int deltaPic, int verticalFov, int deltaSec )
{
		...
		if ( succload ) //load succesfull
		{
			...
			for ( int iterX = 0; iterX < itersX; iterX++ ) //X cyklus
			{
				...
				for ( int iterY = 0; iterY < itersY; iterY++ ) //Y cyklus
				{
					...
					predicted_omega = ( 14.713 - 2.396 * pow( sin( theta ), 2 ) - 1.787 * pow( sin( theta ),
					                    4 ) ) / ( 24. * 60. * 60. ) / ( 360. / 2. / Constants::Pi ); //predicted omega in radians per second
					...
					{
						//crop1
						crop1 = roicrop( pic1.image(), pic1.params().fitsMidX + iterX - itersX / 2 + iterMedian - itersMedian / 2,
						                 pic1.params().fitsMidY + vertikalniskok * ( iterY - itersY / 2 ) + vertikalniShift, set.getcols(), set.getrows() );
						//crop2
						crop2 = roicrop( pic2.image(), pic2.params().fitsMidX + iterX - itersX / 2 + iterMedian - itersMedian / 2,
						                 pic2.params().fitsMidY + vertikalniskok * ( iterY - itersY / 2 ) + vertikalniShift, set.getcols(), set.getrows() );
					...
					picture.at<float>( iterY, ( itersPic - 1 )*itersX - iterPic * itersX - iterX ) = pic1.image().at<ushort>( pic1.params().fitsMidY + vertikalniskok * ( iterY - floor( (
					            double )itersY / 2. ) ) + vertikalniShift, pic1.params().fitsMidX - floor( ( double )itersX / 2. ) + iterX );
				}//Y for cycle end
			}//X for cycle end
		}//load successful
...
double DiffrotMerritFunction( const IPCsettings &set, const std::vector<std::pair<FitsImage, FitsImage>> &pics, int itersX, int itersY, int itersMedian, int strajdPic, int deltaPic, int verticalFov, int deltaSec )
{
	double retVal = 0;
	//------------------------------------------------------------------------
	...
	for ( int iterPic = 0; iterPic < pics.size(); iterPic++ ) //main cycle - going through pairs of pics
	{
		...
		for ( int iterX = 0; iterX < itersX; iterX++ ) //X cyklus
		{
			...
			for ( int iterY = 0; iterY < itersY; iterY++ ) //Y cyklus
			{
				...
				{
					//crop1
					crop1 = roicrop( pics[iterPic].first.image(), pics[iterPic].first.params().fitsMidX + iterX - itersX / 2 + iterMedian - itersMedian / 2,
					                 pics[iterPic].first.params().fitsMidY + vertikalniskok * ( iterY - itersY / 2 ) + vertikalniShift, set.getcols(), set.getrows() );
					//crop2
					crop2 = roicrop( pics[iterPic].second.image(), pics[iterPic].second.params().fitsMidX + iterX - itersX / 2 + iterMedian - itersMedian / 2,
					                 pics[iterPic].second.params().fitsMidY + vertikalniskok * ( iterY - itersY / 2 ) + vertikalniShift, set.getcols(), set.getrows() );
                                     ...
                                     int deltaPic, int verticalFov, int deltaSec )
{
	...
	return DiffrotMerritFunction( set, pics, itersX, itersY, itersMedian, strajdPic, deltaPic, verticalFov, deltaSec );
}
...
double DiffrotMerritFunctionWrapper2( std::vector<double> &arg, const std::vector<std::pair<FitsImage, FitsImage>> &pics, int itersX, int itersY, int itersMedian, int strajdPic,
                                      int deltaPic, int verticalFov, int deltaSec )
{
	IPCsettings set( 32, 32, arg[0], arg[1] );
	set.L2size = 11;
	set.applyWindow = false;
	return DiffrotMerritFunction( set, pics, itersX, itersY, itersMedian, strajdPic, deltaPic, verticalFov, deltaSec );
}