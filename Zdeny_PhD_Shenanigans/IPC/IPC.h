...
inline Point2f ipcinsides(Mat&& sourceimg1, Mat&& sourceimg2, const IPCsettings& set, IPlot2D* plt = nullptr, bool forceshow = false)
{
	...
	if (set.IPCshow) LOG_DEBUG("Cross-power spectrum calculated");
	Mat CrossPower;
	merge(CrossPowerPlanes, 2, CrossPower);
	if (set.applyBandpass)
	{
		CrossPower = bandpass(CrossPower, set.bandpass);
		if (set.IPCshow) LOG_DEBUG("Cross-power spectrum bandpassed");
	}
	if (0)//CORRECT - complex magnitude - input can be real or complex whatever
	{
		...
		if (1)
		{
			...
			if (set.IPCshow) LOG_DEBUG("L3 real min/max: " + to_string(std::get<0>(minmaxReal)) + " / " + to_string(std::get<1>(minmaxReal)));
	...
	if (set.IPCshow) LOG_DEBUG("Inverse fourier of cross-power spectrum calculated");
	...
	if (set.IPCshow) LOG_DEBUG("Phase correlation max: " + to_string(maxR) + " at location: " + to_string(L3peak));
	if (set.IPCshow) LOG_DEBUG("Calculated shift with pixel accuracy: " + to_string(imageshift_PIXEL) + " pixels");
	if (set.IPCshow) LOG_DEBUG("Phase correlation calculated with pixel accuracy");
	...
	if (set.subpixel)
	{
		...
		else
		{
			...
			if (set.IPCshow) LOG_DEBUG("L2Upeak location before iterations: " + to_string(L2Upeak));
			if (set.IPCshow) LOG_DEBUG("L2Upeak location before iterations findCentroid double: " + to_string(findCentroidDouble(L2U)));
			...
			if (!set.iterate)
			{
				...
				if (set.IPCshow)
				{
					...
					showimg(crosshair(L1v, findCentroidDouble(L1) * 2000 / L1.cols), "L1 a4r", true);
				}
			}
			else
			{
				...
				if (set.IPCshow)
				{
					Mat L1v; resize(L1, L1v, cv::Size(2000, 2000), 0, 0, INTER_LINEAR);
				...
				for (int i = 0; i < maxPCit; i++)
				{
					...
					if (set.IPCshow) LOG_DEBUG("L1peak: " + to_string(L1peak));
					...
					if (set.IPCshow) LOG_DEBUG("L1peak findCentroid double delta: " + to_string(findCentroidDouble(L1).x - L1mid.x) + " , " + to_string(findCentroidDouble(L1).y - L1mid.y));
					if (set.IPCshow) LOG_DEBUG("Resulting L2Upeak in this iteration: " + to_string(L2Upeak));
					if ((abs(L1peak.x - L1mid.x) < 0.5) && (abs(L1peak.y - L1mid.y) < 0.5))
					{
						L1 = kirklcrop(L2U, L2Upeak.x, L2Upeak.y, L1size);
						if (set.IPCshow) LOG_DEBUG("Iterative phase correlation accuracy reached");
						if (set.IPCshow) LOG_DEBUG("L2Upeak: " + to_string(L2Upeak));
						if (set.IPCshow || forceshow)
						{
							Mat L1v;
			...
			if (set.IPCshow) LOG_SUCC("Iterative phase correlation calculated with subpixel accuracy");
			if (set.IPCshow) LOG_DEBUG("L3 size: " + to_string(L3.cols) + ", L2 size: " + to_string(L2.cols) + ", L2U size: " + to_string(L2U.cols) + ", L1 size: " + to_string(L1.cols));
			if (set.IPCshow) LOG_DEBUG("Upsample pixel accuracy theoretical maximum: " + to_string(1.0 / (float)set.UC));
			if (set.IPCshow) LOG_DEBUG("Calculated shift with pixel accuracy: " + to_string(imageshift_PIXEL) + " pixels ");
			if (set.IPCshow) LOG_SUCC("Calculated shift with SUBpixel accuracy1: " + to_string(imageshift_SUBPIXEL) + " pixels ");
