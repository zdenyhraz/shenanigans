...
struct Plot2D : AbstractPlot2D
{
	...
	inline Plot2D(std::vector<WindowPlot*>& windowPlotss, QString xlabel, QString ylabel, QString zlabel)
	{
		windowPlots = &windowPlotss;
		windowPlots->push_back(new WindowPlot());
		index = windowPlots->size() - 1;
		(*windowPlots)[index]->show();
		(*windowPlots)[index]->ui.widget->addGraph();
		...
		(*windowPlots)[index]->ui.widget->xAxis->setLabel(xlabel);//give the axes some labels
		(*windowPlots)[index]->ui.widget->yAxis->setLabel(ylabel);//give the axes some labels
		colorMap = new QCPColorMap((*windowPlots)[index]->ui.widget->xAxis, (*windowPlots)[index]->ui.widget->yAxis);//set up the QCPColorMap
		...
		colorScale = new QCPColorScale((*windowPlots)[index]->ui.widget);//add a color scale
		...
		marginGroup = new QCPMarginGroup((*windowPlots)[index]->ui.widget);//make sure the axis rect and color scale synchronize their bottom and top margins (so they line up)
	...
	inline void setAxisNames(std::string xlabel, std::string ylabel, std::string zlabel) override
	{
		...
		(*windowPlots)[index]->ui.widget->xAxis->setLabel(qxlabel);
		(*windowPlots)[index]->ui.widget->yAxis->setLabel(qylabel);
