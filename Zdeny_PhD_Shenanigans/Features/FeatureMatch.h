...
#pragma omp parallel for
  for (int pic = 0; pic < piccnt - 1; pic++)
  {
    ...
    for (int i = 0; i < matches.size(); i++)
    {
      ...
      LOG_DEBUG("Calculated feature shift[{}] =[{},{}], magn={}, wmagn={}, descdist={}, spd={} km/s", i, shifts[i].x, shifts[i].y, magnitude(shifts[i]), data.magnitudeweight * magnitude(shifts[i]),
                matches[i].distance, speeds[i]);
    }
