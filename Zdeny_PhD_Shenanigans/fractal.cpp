...
Mat computeFractal(Fractalset& fractalset)
{
	...
	for (int r = 0; r < fractalset.fractalHeight; r++)
	{
		...
		{
			complex<double> C = (double)c / (fractalset.fractalWidth)*(fractalset.xmax - fractalset.xmin) + fractalset.xmin + 1.i * (-(double)r / (fractalset.fractalHeight)*(fractalset.ymax - fractalset.ymin) + fractalset.ymax);
			complex<double> Z = startZ;
			double M = 0.;
			int finaliter = 0;
