...
Point2d phasecorrel(const Mat& sourceimg1In, const Mat& sourceimg2In, IPCsettings& IPC_set, const Mat& windowMat, const Mat& bandpassMat, double* corrQuality)
{
	...
	if (IPC_set.IPCspeak) std::cout << "Phase correlation max: " << maxR << " at location: " << L3peak << std::endl;
	...
	if (IPC_set.IPCspeak) std::cout << "Calculated shiftX with pixel accuracy: " << imageshiftX_PIXEL << " pixels" << std::endl;
	if (IPC_set.IPCspeak) std::cout << "Calculated shiftY with pixel accuracy: " << imageshiftY_PIXEL << " pixels" << std::endl;
	if (IPC_set.IPCshow) { Mat L3v; resize(L3, L3v, cv::Size(2000, 2000), 0, 0, INTER_NEAREST); showimg(crosshair(L3v, Point2d(round((double)(L3peak.x) * 2000. / (double)L3.cols), round((double)(L3peak.y) * 2000. / (double)L3.rows))), "L3", true, 0, 1); }
	if (IPC_set.IPCshow) 
	{ 
	...
	{
		if (IPC_set.IPCspeak) cout << "degenerate peak, results might be inaccurate!" << endl;
		//cout << "-!";
		IPC_set.subpixel = false;
	}
	if (IPC_set.subpixel)
	{
		...
		Mat L2U;
		if (IPC_set.interpolate)
			resize(L2, L2U, L2.size()*IPC_set.UC, 0, 0, INTER_LINEAR);
		else
			resize(L2, L2U, L2.size()*IPC_set.UC, 0, 0, INTER_NEAREST);
		Point2d L2Umid;
		L2Umid.x = floor((double)(L2U.cols) / 2.0);
		L2Umid.y = floor((double)(L2U.rows) / 2.0);
		if (IPC_set.IPCshow) { Mat L2v; resize(L2, L2v, cv::Size(2000, 2000), 0, 0, INTER_NEAREST); showimg(crosshair(L2v, L2mid * 2000 / L2.cols), "L2", true); }
		if (IPC_set.IPCshow) { Mat L2Uv; resize(L2U, L2Uv, cv::Size(2000, 2000), 0, 0, INTER_LINEAR); showimg(crosshair(L2Uv, L2Umid * 2000 / L2U.cols), "L2U", true, 0, 1); }
		Point2d L2Upeak;
		L2Upeak.x = floor((double)(L2U.cols) / 2.0);
		L2Upeak.y = floor((double)(L2U.rows) / 2.0);
		if (IPC_set.IPCspeak) std::cout << "L2Upeak location before iterations: " << L2Upeak << std::endl;
		if (IPC_set.IPCspeak) std::cout << "L2Upeak location before iterations findCentroid double: " << findCentroidDouble(L2U) << std::endl;
		int L1size = std::round((double)L2U.cols*IPC_set.L1ratio);
		...
		if (!IPC_set.iterate)
		{
			...
			if (IPC_set.IPCshow) { Mat L1v; resize(L1, L1v, cv::Size(2000, 2000), 0, 0, INTER_NEAREST); showimg(crosshair(L1v, findCentroidDouble(L1) * 2000 / L1.cols), "L1 a4r", true); }
		}
		else
		{
			...
			L1mid.x = floor((double)(L1.cols) / 2.);
			L1mid.y = floor((double)(L1.rows) / 2.);
			if (IPC_set.IPCshow) { Mat L1v; resize(L1, L1v, cv::Size(2000, 2000), 0, 0, INTER_LINEAR); showimg(crosshair(L1v, L1mid * 2000 / L1.cols), "L1 be4", true); }
			int maxPCit = 50;
			for (int i = 0; i < maxPCit; i++)
			{
				if (IPC_set.IPCspeak) std::cout << "====================================" << " iterace " << i << " ====================================" << std::endl;
				L1 = kirklcrop(L2U, L2Upeak.x, L2Upeak.y, L1size);
				Point2d L1peak = findCentroidDouble(L1);
				if (IPC_set.IPCspeak) std::cout << "L1peak: " << L1peak << std::endl;
				...
				{
					if (IPC_set.IPCspeak) cout << "breaking out of pc iterations, centroid diverges" << endl;
					//cout << "!";
					break;
				}
				if (IPC_set.IPCspeak) std::cout << "L1peak findCentroid double delta: " << findCentroidDouble(L1).x - L1mid.x << " , " << findCentroidDouble(L1).y - L1mid.y << std::endl;
				if (IPC_set.IPCspeak) std::cout << "Resulting L2Upeak in this iteration: " << L2Upeak << std::endl;
				if ((abs(L1peak.x - L1mid.x) < 0.5) && (abs(L1peak.y - L1mid.y) < 0.5))
				{
					L1 = kirklcrop(L2U, L2Upeak.x, L2Upeak.y, L1size);
					if (IPC_set.IPCspeak) std::cout << "Iterative PC accuracy reached <<<" << std::endl;
					if (IPC_set.IPCspeak) std::cout << "L2Upeak: " << L2Upeak << std::endl;
					if (IPC_set.IPCshow) { Mat L1v; resize(L1, L1v, cv::Size(2000, 2000), 0, 0, INTER_LINEAR); showimg(crosshair(L1v, L1mid * 2000 / L1.cols), "L1 a4r", true); }
					if (IPC_set.IPCspeak) std::cout << "===================================================================================" << std::endl;
					break;
				}
				if (i == (maxPCit - 1))
				{
					if (IPC_set.IPCspeak) cout << "PC out of iterations - diverges" << endl;
					//cout << "!";
				}
			}
			imageshiftX_SUBPIXEL = (double)L3peak.x - (double)L3mid.x + 1.0 / (double)IPC_set.UC*((double)L2Upeak.x - (double)L2Umid.x + findCentroidDouble(L1).x - (double)L1mid.x);//image shift in L3 - final
			imageshiftY_SUBPIXEL = (double)L3peak.y - (double)L3mid.y + 1.0 / (double)IPC_set.UC*((double)L2Upeak.y - (double)L2Umid.y + findCentroidDouble(L1).y - (double)L1mid.y);
		}

		if (IPC_set.IPCspeak) std::cout << "phase correlation calculated with subpixel accuracy" << std::endl;
		if (IPC_set.IPCspeak) std::cout << "Calculated shiftX with pixel accuracy: " << imageshiftX_PIXEL << " pixels " << std::endl;
		if (IPC_set.IPCspeak) std::cout << "Calculated shiftY with pixel accuracy: " << imageshiftY_PIXEL << " pixels " << std::endl;
		if (IPC_set.IPCspeak) std::cout << "Calculated shiftX with SUBpixel accuracy1: " << imageshiftX_SUBPIXEL << " pixels " << std::endl;
		if (IPC_set.IPCspeak) std::cout << "Calculated shiftY with SUBpixel accuracy1: " << imageshiftY_SUBPIXEL << " pixels " << std::endl;
		if (IPC_set.IPCspeak) std::cout << "L3 size: " << L3.size << ", L2 size: " << L2.size << ", L2U size: " << L2U.size << ", L1 size: " << L1.size << std::endl;
		if (IPC_set.IPCspeak) std::cout << "Upsample pixel accuracy theoretical maximum: " << 1.0 / (double)IPC_set.UC << std::endl;

		output.x = imageshiftX_SUBPIXEL;
		output.y = imageshiftY_SUBPIXEL;
	}
	return output;
}
