...
inline std::vector<double> InverseMapping(
    const std::vector<std::vector<double>>& trialInputs, const std::vector<std::vector<double>>& trialOutputs, const std::vector<double>& desiredOutput, int degree)
{
...
inline std::vector<double> InverseMappingTest(const std::vector<double>& arg)
{
  auto result = arg;
  auto N = arg.size();
