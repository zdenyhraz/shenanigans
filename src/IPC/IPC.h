...
inline void ipcsaveimg(const char* filename, const cv::Mat& img, const IPCsettings& set)
{
  saveimg(set.savedir + std::to_string(set.savecntr) + "_" + filename, img, false, set.savesize, true, 0, 1);
}

inline cv::Point2f ipccore(cv::Mat&& sourceimg1, cv::Mat&& sourceimg2, const IPCsettings& set, bool forceshow = false)
{
  sourceimg1.convertTo(sourceimg1, CV_32F, 1. / 65535);
  sourceimg2.convertTo(sourceimg2, CV_32F, 1. / 65535);

  std::vector<cv::Mat> showMatsGRS; // grayscale
  std::vector<cv::Mat> showMatsCLR; // color
  ...
  cv::Mat DFT1 = fourier(std::move(sourceimg1));
  cv::Mat DFT2 = fourier(std::move(sourceimg2));

  cv::Mat planes1[2];
  cv::Mat planes2[2];
  cv::Mat CrossPowerPlanes[2];
  ...
  if (!set.crossCorrel)
  {
    cv::Mat magnre, magnim;
    pow(CrossPowerPlanes[0], 2, magnre);
    pow(CrossPowerPlanes[1], 2, magnim);
    cv::Mat normalizationdenominator = magnre + magnim;
  ...
  {
    cv::Mat L3complex;
    dft(CrossPower, L3complex, cv::DFT_INVERSE + cv::DFT_SCALE);
    cv::Mat L3planes[2];
  ...
  {
    dft(CrossPower, L3, cv::DFT_INVERSE + cv::DFT_SCALE + cv::DFT_REAL_OUTPUT);
    if (set.speak > IPCsettings::Errors)
    {
  ...
  L3 = quadrantswap(L3);
  // normalize(L3, L3, 0, 1, cv::NORM_MINMAX); //not needed, can see the quality by value at peak
  ...
  if (set.speak > IPCsettings::Errors or forceshow)
  {
    ...
    showMatsCLR.push_back(crosshair(L3vz, cv::Point2f(L3vz.cols / 2, L3vz.rows / 2)));
  ...
  if (set.subpixel)
  {
    ...
    while (!converged)
    {
      ...
      {
        cv::Point2f imageshift_SUBPIXEL;
        cv::Mat L2 = roicrop(L3, L3peak.x, L3peak.y, L2size, L2size);
        cv::Mat L2U;

        if (set.interpolate)
          resize(L2, L2U, L2.size() * set.UC, 0, 0, cv::INTER_LINEAR);
        else
          resize(L2, L2U, L2.size() * set.UC, 0, 0, cv::INTER_NEAREST);
...
inline cv::Point2f phasecorrel(const cv::Mat& sourceimg1In, const cv::Mat& sourceimg2In, const IPCsettings& set, bool forceshow = false)
{
  cv::Mat sourceimg1 = sourceimg1In.clone();
  cv::Mat sourceimg2 = sourceimg2In.clone();

  return ipccore(std::move(sourceimg1), std::move(sourceimg2), set, forceshow);
}

inline cv::Point2f phasecorrel(cv::Mat&& sourceimg1, cv::Mat&& sourceimg2, const IPCsettings& set, bool forceshow = false)
{
  return ipccore(std::move(sourceimg1), std::move(sourceimg2), set, forceshow);
}
